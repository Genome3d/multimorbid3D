...
def parse_args():
    ...
    parser.add_argument(
        '--string-version',
        choices = get_stringdb_versions(), action='append',
...
if __name__=='__main__':
    ...
    if not args.genes and not args.snps and not args.trait and not args.pmid:
        sys.exit('\nFATAL: One of --genes, --snps, --trait, or --pmid is required.\nExiting.\n')
    start_time = time.time()
    if args.genes and (args.snps or args.trait or args.pmid):
        sys.exit('\nOnly one of --genes, --snps, --trait, or --pmid is required.\nExiting.\n')
